...
function hasTruncation(value: unknown): boolean {
  ...
  if ((value as Record<string, unknown>).__trackmcp_truncated === true)
    return true;
...
function Badge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-1 text-[10px] font-medium ${tone === "warn" ? "border-amber-200 bg-amber-50 text-amber-800" : tone === "good" ? "border-brand/20 bg-brand-soft text-brand-strong" : "border-line bg-paper text-muted"}`}
    >
      {children}
    </span>
  );
...
function eventStatus(event: TraceEvent): {
  label: string;
...
function observationLabel(event: TraceEvent): string {
  return event.observation_source === "client"
    ? "Client observed"
    : event.observation_source === "server"
      ? "Server observed"
      : "Legacy/Unknown";
}

function correlationSourceLabel(
  source: TraceEvent["correlation_handle_source"],
): string {
...
function EventCard({ event, index }: { event: TraceEvent; index: number }) {
  ...
  return (
    <article className="relative border-l-2 border-line-strong pb-6 pl-6 last:pb-0">
      ...
      <div className="rounded-xl border border-line bg-white p-4 shadow-[0_1px_2px_rgba(31,43,36,0.03)]">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted">
          ...
          {event.retry_number !== null && (
            <span>Retry {event.retry_number}</span>
          )}
          {event.transport && <span>{event.transport}</span>}
          {event.direction && <span>{event.direction}</span>}
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          <Badge>{observationLabel(event)}</Badge>
          {event.correlation_handle_source && (
            <Badge>
              {correlationSourceLabel(event.correlation_handle_source)}
            </Badge>
          )}
          {redacted && (
            <Badge>
          ...
          {event.payload_policy === "full" && (
            <Badge tone="warn">Full mode</Badge>
          )}
          {!event.payload && event.payload_policy !== "metadata" && (
            <Badge>Payload not captured</Badge>
          )}
        </div>
        <dl className="mt-4 grid gap-2 text-xs sm:grid-cols-2">
          {event.client_name && (
            <div>
              ...
              <dd className="mt-0.5 text-body">
                {event.client_name}
                {event.client_version ? ` ${event.client_version}` : ""}
              </dd>
            </div>
          )}
          {event.server_id && (
            <div>
              <dt className="text-faint">Server boundary</dt>
              <dd className="mt-0.5 text-body">
                {event.server_id}
                {event.server_version ? ` · ${event.server_version}` : ""}
              </dd>
            </div>
          )}
          {event.request_id && (
            <div>
              ...
              <dd className="mt-0.5 truncate font-mono text-body">
                {event.request_id}
              </dd>
            </div>
          )}
          {event.workflow_id && (
            <div>
              ...
              <dd className="mt-0.5 font-mono text-body">
                {event.workflow_id}
              </dd>
            </div>
          )}
        ...
        {event.payload && (
          <details className="mt-4 border-t border-line pt-3">
            <summary className="cursor-pointer text-xs font-medium text-muted">
              View bounded payload
            </summary>
            <pre className="mt-3 max-h-72 overflow-auto rounded-lg bg-[#101713] p-3 font-mono text-[11px] leading-relaxed text-white/85">
...
export function TraceExplorer({
  ...
  useEffect(() => {
    ...
    const query = sessionId
      ...
      : `correlation_handle=${encodeURIComponent(correlationHandle || "")}`;
    void fetch(`/api/v1/traces?${query}&limit=200`, {
      cache: "no-store",
      credentials: "include",
      signal: controller.signal,
    })
      .then(async (result) => {
        const body = (await result.json()) as TraceResponse & {
          error?: string;
        };
        if (!result.ok)
          throw new Error(
            result.status === 401
              ? "Your session is no longer authorized. Sign in again, then retry."
              : body.error || "Could not load this trace.",
          );
        setResponse(body);
      })
  ...
  }, [sampleMode, sessionId, correlationHandle, retryToken]);
  ...
  const copySession = async () => {
    try {
      await navigator.clipboard.writeText(sessionId || correlationHandle || "");
      setCopied(true);
  ...
  return (
    <div>
      <div className="mb-7 flex flex-wrap items-start justify-between gap-4">
        <div>
          <button
            type="button"
            onClick={onBack}
            className="mb-4 inline-flex cursor-pointer items-center gap-1.5 text-xs font-medium text-muted hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand"
          >
            <ArrowLeft size={14} />
            Back to {resolvedOriginLabel.toLowerCase()}
          </button>
          <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-brand">
            {sampleMode
              ? "Sample / Trace Explorer"
              : `${resolvedOriginLabel} / Trace Explorer`}
          </p>
          <h2 className="mt-2 text-2xl font-medium tracking-[-0.03em] text-ink">
            Trace Explorer
          </h2>
          <p className="mt-2 max-w-2xl text-sm text-muted">
            One bounded event at a time, in observed order. Payload details
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-line bg-white px-4 py-3">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] uppercase tracking-[0.12em] text-faint">
            {sessionId ? "Session ID" : "Correlation handle"}
          </p>
          <p className="mt-1 truncate font-mono text-xs text-body">
            {sessionId || correlationHandle}
          </p>
        </div>
        <button
          type="button"
          aria-label="Copy trace identifier"
      ...
      {sampleMode ? (
        ...
        <div
          className="flex flex-wrap items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-700"
          role="alert"
        >
          ...
          <button
            type="button"
            onClick={retry}
            className="shrink-0 cursor-pointer font-semibold underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600"
          >
            Retry
          </button>
        </div>
        ...
        <div className="grid min-h-[280px] place-items-center rounded-xl border border-dashed border-line-strong bg-white p-8 text-center">
          <div>
            ...
            <p className="mt-2 text-sm text-muted">
              The session may have expired, or it may belong to another
              workspace.
            </p>
          </div>
        </div>
        ...
        <>
          ...
          {(response.correlation_quality === "missing" ||
            ...
            <div className="mb-5 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-800">
              ...
              <span>
                This trace has degraded or mixed correlation. A request ID alone
                does not establish a reliable relationship between events.
              </span>
            </div>
          )}
          {response.truncated && (
            <div className="mb-5 flex flex-wrap items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
              ...
              response is capped. Narrow the time range or scope to inspect more
              precisely.
            </div>
          )}
