...
export function IntegrationChecklist({ hasKey, working, onGenerateKey, onRefresh, onComplete, onDismiss, checking, checkStatus }: {
  ...
  return <section className="mb-7 border border-[#dfe7e2] bg-white">
    <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#e7ebe8] px-5 py-4 sm:px-6">
      <div><p className="text-[11px] font-semibold uppercase tracking-[0.15em] text-[#14956f]">First connection</p><h2 className="mt-1 text-[22px] font-semibold tracking-[-0.025em] text-[#202020]">Connect your MCP server</h2><p className="mt-1 text-[14px] text-[#707570]">Generate a key, install the SDK, and send one event to observe the server boundary. Payloads are redacted and bounded by default.</p></div>
    ...
    <div className="grid gap-0 divide-y border-b border-[#e7ebe8] md:grid-cols-4 md:divide-x md:divide-y-0">
      ...
      <Step number="03" title="Wrap your server" done={false}><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1"><a href="/docs/typescript" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[12px] font-medium text-[#168c69] hover:underline">View SDK guide <ExternalLink size={12} /></a><a href="/docs/reference#privacy" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[12px] font-medium text-[#168c69] hover:underline">Privacy defaults <ExternalLink size={12} /></a></div></Step>
...
function Step({ number, title, done, children }: { number: string; title: string; done: boolean; children: React.ReactNode }) {
  return <div className="px-5 py-5 sm:px-6"><div className="flex items-center gap-2"><span className="font-mono text-[11px] text-[#9ba49e]">{number}</span><h3 className="text-[14px] font-semibold text-[#333]">{title}</h3>{done && <Check size={15} className="text-[#14956f]" />}</div>{children}</div>;
}
