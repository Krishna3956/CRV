...
export function DashboardApp({
  ...
  useEffect(() => {
    ...
    const requestedHandle = params.get("view") === "trace" ? params.get("correlation_handle") : null;
    if (requestedTrace || requestedHandle) {
      // URL state is read after hydration so the server-rendered dashboard stays stable.
      window.setTimeout(() => { setTraceSessionId(requestedTrace); setTraceCorrelationHandle(requestedHandle); setView("sessions"); }, 0);
    }
  }, []);
  const openTrace = (sessionId: string, correlationHandle?: string | null) => {
    ...
    setView("sessions");
    const params = new URLSearchParams({ view: "trace", session_id: sessionId });
...
function ViewContent({ view, analytics, keys, working, onGenerateKey, onRevokeKey, onViewChange, onViewTrace, isSample }: { view: View; analytics: Analytics; keys: Key[]; working: boolean; onGenerateKey: () => void; onRevokeKey: (id: string) => void; onViewChange: (view: View) => void; onViewTrace: (sessionId: string, correlationHandle?: string | null) => void; isSample: boolean }) {
...
function WorkflowsView({ analytics, sessionsOnly, onViewTrace, isSample }: { analytics: Analytics; sessionsOnly: boolean; onViewTrace: (sessionId: string, correlationHandle?: string | null) => void; isSample: boolean }) {
  ...
  return <>
    ...
    <div className="grid gap-4 sm:grid-cols-3"><Metric label="Workflows started" value={fmt(outcomes.reduce((sum, outcome) => sum + outcome.started, 0))} icon={Layers3} /><Metric label="Completed" value={fmt(outcomes.reduce((sum, outcome) => sum + outcome.completed, 0))} icon={Check} /><Metric label="Completion rate" value={percent(analytics.completion_rate)} icon={Target} /></div>
...
function SessionsView({ analytics, onViewTrace, isSample }: { analytics: Analytics; onViewTrace: (sessionId: string, correlationHandle?: string | null) => void; isSample: boolean }) {
  return <><PageIntro title="Sessions" desc="Inspect each observed MCP connection: who connected, how long it lasted, and which signals were recorded." /><div className="grid gap-4 sm:grid-cols-3"><Metric label="Active connections" value={fmt(analytics.sessions)} icon={Activity} /><Metric label="Tool calls" value={fmt(analytics.tool_calls)} icon={Wrench} /><Metric label="Session completion" value={percent(analytics.completion_rate)} icon={Check} /></div><div className="mt-6"><Panel title="Connection log" subtitle="One row per observed MCP session"><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="border-b border-line text-[10px] font-semibold uppercase tracking-[0.08em] text-faint"><tr><th className="pb-3">Client</th><th className="pb-3">Session</th><th className="pb-3">Started</th><th className="pb-3">Duration</th><th className="pb-3">Calls</th><th className="pb-3">Correlation</th><th className="pb-3">Status</th><th className="pb-3">Trace</th></tr></thead><tbody className="divide-y divide-line">{analytics.workflows.length ? analytics.workflows.map((session) => <tr key={session.session_id}><td className="py-4"><div className="flex items-center gap-2.5"><ClientTile name={clientMark(session.client_name)} className="h-7 w-7 rounded-full" /><span className="text-ink">{session.client_name}</span></div></td><td className="py-4 font-mono text-xs text-muted">{session.session_id.slice(0, 18)}...</td><td className="py-4 text-xs text-muted">{new Date(session.started_at).toLocaleString()}</td><td className="py-4 text-muted">{session.duration_ms}ms</td><td className="py-4 text-muted">{session.calls}</td><td className="py-4 text-xs text-muted"><span>{correlationLabel(session.correlation_quality)}</span>{session.correlation_handle && <code className="mt-1 block truncate font-mono text-[10px] text-faint">{session.correlation_handle}</code>}</td><td className="py-4"><span className={session.completed ? "text-brand-strong" : "text-amber-700"}>{session.completion_source === "workflow_events" ? (session.completed ? "Explicitly completed" : "Explicitly failed") : session.completed ? "Ended after successful call" : "Ended without explicit outcome"}</span><span className="mt-1 block text-[10px] text-faint">{session.completion_source === "workflow_events" ? "explicit workflow" : session.completion_source === "session_heuristic" ? "session heuristic" : "no signal"}</span></td><td className="py-4">{isSample ? <span className="text-[11px] text-faint">Sample only</span> : <button onClick={() => onViewTrace(session.session_id, session.correlation_handle)} className="text-xs font-medium text-brand-strong hover:underline">View trace</button>}</td></tr>) : <tr><td colSpan={8} className="py-10 text-center text-sm text-muted">No sessions have been received yet.</td></tr>}</tbody></table></div></Panel></div></>;
}
