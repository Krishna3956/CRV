...
import {
  Activity, AlertTriangle, BarChart3, Check, ChevronDown, Clock3, Clipboard, ExternalLink,
...
const DEMO_ANALYTICS: Analytics = {
  range_days: 30, total_events: 12864, protocol_events: 4218, catalog_events: 1124, protocol_versions: ["2025-06-18", "2025-11-25"], transports: ["stdio", "streamable_http", "sse"], methods: ["initialize", "notifications/initialized", "tools/list", "tools/call", "resources/list", "resources/read", "prompts/list", "prompts/get", "ping", "completion/complete"], tool_calls: 8642, sessions: 1248, errors: 286, completion_rate: 0.87,
  funnel: { connections: 1248, discovered_tools: 18, tool_calls: 8642, successful_calls: 8356 },
...
export function DashboardApp({
  ...
  setupRequired: boolean; setupDetails: SetupDetails; onboardingMode?: boolean; onGenerateKey: () => void; onRevokeKey: (id: string) => void; onDismissKey: () => void; onRefresh: (days?: string) => void; onCreateWorkspace: (details?: SetupDetails) => void; onSignOut: () => void;
}) {
  ...
  const goTo = (next: View) => {
    setOnboarding(false);
    setView(next);
    if (window.location.pathname === "/dashboard/onboarding") window.history.pushState({}, "", "/dashboard");
  };
  ...
  return <div className="min-h-screen bg-[#fafafa] font-display text-ink">
    ...
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] border-r border-[#e7e7e7] bg-[#fcfcfc] lg:flex lg:flex-col">
      ...
      <nav className="mt-7 px-3">{nav.map((item, index) => <div key={item.id}>{item.group && <p className={`${index ? "mt-5" : ""} mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.12em] text-faint`}>{item.group}</p>}<button onClick={() => goTo(item.id)} className={`mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13.5px] transition-colors ${view === item.id ? "bg-ink text-white shadow-sm" : "text-muted hover:bg-paper hover:text-ink"}`}><item.icon size={16} strokeWidth={view === item.id ? 2.2 : 1.8} />{item.label}</button></div>)}</nav>
      <div className="mt-auto border-t border-line p-3"><button onClick={() => goTo("settings")} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13.5px] ${view === "settings" ? "bg-paper text-ink" : "text-muted hover:bg-paper hover:text-ink"}`}><Settings size={16} />Configure</button><button onClick={onSignOut} className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-[13.5px] text-muted hover:bg-paper hover:text-ink"><LogOut size={16} />Sign out</button><div className="mt-3 flex items-center gap-2 border-t border-line px-2 pt-3"><span className="grid h-7 w-7 place-items-center rounded-full bg-brand text-[10px] font-bold text-white">{email.slice(0, 1).toUpperCase()}</span><span className="min-w-0 truncate text-[11.5px] text-muted">{email}</span></div></div>
    </aside>
    <div className="lg:pl-[248px]">
<header className="sticky top-0 z-20 flex h-[58px] items-center justify-between border-b border-[#e7e7e7] bg-white/95 px-5 backdrop-blur sm:px-8"><div><p className="text-[10px] font-medium uppercase tracking-[0.14em] text-[#9a9f9c]">{workspace?.name || "TrackMCP"}</p><h1 className="mt-0.5 text-[19px] font-semibold tracking-[-0.025em] text-[#202020]">{activeNav?.label}</h1></div><div className="flex items-center gap-2"><select value={range} onChange={(event) => { setRange(event.target.value); onRefresh(event.target.value); }} className="hidden border border-[#d8dfdb] bg-white px-3 py-2 text-[12px] text-[#4c5550] outline-none sm:block"><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option></select><button type="button" role="switch" aria-checked={showSample} aria-label={showSample ? "Showing sample data. Switch to my data" : "Showing my data. Switch to sample data"} onClick={() => setSampleMode((current) => !(current ?? !hasLiveData))} className="inline-flex min-h-9 items-center gap-2 border border-[#d8dfdb] px-2.5 py-2 text-[12px] font-medium text-[#4c5550] hover:bg-[#f6f8f6]"><span aria-hidden="true" className={`relative inline-flex h-4 w-7 shrink-0 rounded-full transition-colors ${showSample ? "bg-[#159b73]" : "bg-[#c9d1cc]"}`}><span className={`absolute top-0.5 left-0 h-3 w-3 rounded-full bg-white shadow-sm transition-transform ${showSample ? "translate-x-3.5" : "translate-x-0.5"}`} /></span><span className="whitespace-nowrap">{showSample ? "Sample data" : "My data"}</span></button><button onClick={() => onRefresh(range)} title="Refresh data" className="grid h-9 w-9 place-items-center border border-[#d8dfdb] text-[#69736d] hover:bg-[#f6f8f6]"><RefreshCw size={15} /></button><button onClick={() => goTo("settings")} className="grid h-9 w-9 place-items-center border border-[#d8dfdb] text-[#69736d] hover:bg-[#f6f8f6]"><Settings size={15} /></button></div></header>
      <div className="border-b border-line bg-white px-5 py-2.5 lg:hidden"><div className="flex gap-1 overflow-x-auto">{[...nav, { id: "settings" as View, label: "Configure", icon: Settings }].map((item) => <button key={item.id} onClick={() => goTo(item.id)} className={`shrink-0 rounded-md px-3 py-1.5 text-xs font-medium ${view === item.id ? "bg-ink text-white" : "text-muted hover:bg-paper"}`}>{item.label}</button>)}</div></div>
<main className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8 sm:py-9">{error && <div className="mb-6 flex items-center gap-2 border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"><AlertTriangle size={16} />{error}</div>}{workspace && needsIntegration && showOnboarding && <IntegrationChecklist hasKey={hasActiveKey} working={working} checking={checking} checkStatus={checkStatus} onGenerateKey={onGenerateKey} onRefresh={checkForData} onComplete={openOnboarding} onDismiss={() => setShowOnboarding(false)} />}{newKey && view === "settings" && <div className="mb-6 border border-brand/30 bg-brand-soft/40 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.1em] text-brand-strong">API key created</p><p className="mt-1 text-sm text-muted">Copy it now. The complete secret will not be shown again.</p></div><button onClick={onDismissKey} aria-label="Dismiss new API key notice" className="rounded-md p-1 text-muted hover:bg-white/70"><X size={16} /></button></div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><code className="min-w-0 flex-1 overflow-x-auto rounded-lg bg-ink px-4 py-3 font-mono text-sm text-white">{`${newKey.slice(0, 10)}••••••••••••`}</code><button onClick={copyKey} className="inline-flex items-center justify-center gap-2 rounded-lg border border-line-strong bg-white px-4 py-3 text-sm font-medium text-ink">{copied ? <Check size={15} className="text-brand" /> : <KeyRound size={15} />}{copied ? "Copied" : "Copy key"}</button></div></div>}{onboarding ? <DashboardOnboarding workspace={workspace} setupDetails={setupDetails} working={working} error={error} hasKey={hasActiveKey} newKey={newKey} onCreateWorkspace={onCreateWorkspace} onGenerateKey={onGenerateKey} onOpenDashboard={() => goTo("overview")} /> : !workspace ? <SetupState working={working} onCreateWorkspace={onCreateWorkspace} /> : view === "settings" ? <SettingsView keys={keys} working={working} onGenerateKey={onGenerateKey} onRevokeKey={onRevokeKey} /> : !displayedAnalytics ? <EmptyState onSettings={() => goTo("settings")} onDemo={() => setSampleMode(true)} /> : <ViewContent view={view} analytics={displayedAnalytics} keys={keys} working={working} onGenerateKey={onGenerateKey} onRevokeKey={onRevokeKey} onViewChange={goTo} />}</main>
    </div>
  </div>;
}
...
function DashboardOnboarding({ workspace, setupDetails, working, error, hasKey, newKey, onCreateWorkspace, onGenerateKey, onOpenDashboard }: { workspace: Workspace | null; setupDetails: SetupDetails; working: boolean; error: string; hasKey: boolean; newKey: string; onCreateWorkspace: (details: SetupDetails) => void; onGenerateKey: () => void; onOpenDashboard: () => void }) {
  ...
  return <div className="min-h-[calc(100vh-58px)] bg-white">
    <div className="mx-auto max-w-[980px] px-5 py-8 sm:px-10 sm:py-12">
      ...
      {!workspace && <section className="mt-8 max-w-[720px] border border-line bg-paper p-5"><p className="text-base font-semibold text-ink">Tell us about yourself</p><p className="mt-1 text-sm text-muted">These details personalize your account and server insights.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={details.first_name} onChange={(event) => setDetails({ ...details, first_name: event.target.value })} placeholder="First name" className="border border-line-strong bg-white px-3 py-2.5 text-sm outline-none focus:border-brand" /><input value={details.last_name} onChange={(event) => setDetails({ ...details, last_name: event.target.value })} placeholder="Last name" className="border border-line-strong bg-white px-3 py-2.5 text-sm outline-none focus:border-brand" /><input value={details.company_name} onChange={(event) => setDetails({ ...details, company_name: event.target.value })} placeholder="Company name" className="border border-line-strong bg-white px-3 py-2.5 text-sm outline-none focus:border-brand sm:col-span-2" /></div></section>}
...
function WorkflowsView({ analytics, sessionsOnly }: { analytics: Analytics; sessionsOnly: boolean }) {
  if (sessionsOnly) {
    return <SessionsView analytics={analytics} />;
  }
  const outcomes = [...analytics.outcomes].sort((a, b) => (b.started || 0) - (a.started || 0));
  return <>
    <PageIntro title="Workflows" desc="Measure the multi-step jobs your MCP server helps complete, not just individual requests." />
    <div className="grid gap-4 sm:grid-cols-3">
      <Metric label="Workflows started" value={fmt(outcomes.reduce((sum, outcome) => sum + outcome.started, 0))} icon={Layers3} />
      <Metric label="Completed" value={fmt(outcomes.reduce((sum, outcome) => sum + outcome.completed, 0))} icon={Check} />
      <Metric label="Completion rate" value={percent(analytics.completion_rate)} icon={Target} />
    </div>
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)]">
      <Panel title="Outcome journeys" subtitle="A workflow is the user or agent goal across several tool calls">
        <div className="mt-3 divide-y divide-line">{outcomes.length ? outcomes.map((outcome) => {
          const completion = outcome.started ? outcome.completed / outcome.started : 0;
          return <div key={outcome.name} className="py-4">
            <div className="flex items-center justify-between gap-4"><div><p className="font-mono text-sm text-ink">{outcome.name}</p><p className="mt-1 text-xs text-muted">{fmt(outcome.started)} started · {fmt(outcome.failed)} failed</p></div><span className="text-sm font-semibold text-brand-strong">{Math.round(completion * 100)}%</span></div>
            <div className="mt-3 h-2 bg-mist"><div className="h-full bg-brand" style={{ width: `${Math.max(2, completion * 100)}%` }} /></div>
            <div className="mt-2 flex gap-4 text-[11px] text-muted"><span>{fmt(outcome.completed)} completed</span><span>{fmt(outcome.failed)} failed</span></div>
          </div>;
        }) : <p className="py-8 text-sm text-muted">No workflow outcomes have been recorded yet.</p>}</div>
      </Panel>
      <Panel title="How to read this" subtitle="The signal that matters to the business">
        <div className="mt-5 space-y-5 text-sm leading-relaxed text-muted"><p><strong className="font-medium text-ink">Sessions</strong> tell you whether a connection stayed healthy.</p><p><strong className="font-medium text-ink">Workflows</strong> tell you whether the user reached the intended result.</p><p><strong className="font-medium text-ink">Failures</strong> show where an otherwise valid tool sequence did not finish.</p></div>
      </Panel>
    </div>
    <div className="mt-6"><Panel title="Observed workflow traces" subtitle="Recent tool sequences attached to a connection"><div className="mt-3 divide-y divide-line">{analytics.workflows.length ? analytics.workflows.slice(0, 8).map((workflow) => <div key={workflow.session_id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="font-mono text-xs text-ink">{workflow.tools.filter(Boolean).join(" → ") || "No tool sequence"}</p><p className="mt-1 text-[11px] text-muted">{workflow.client_name} · {workflow.calls} calls · {workflow.duration_ms}ms</p></div><span className={`text-[11px] font-medium ${workflow.completed ? "text-brand-strong" : "text-amber-700"}`}>{workflow.completed ? "Completed" : "Stopped"}</span></div>) : <p className="py-8 text-sm text-muted">No workflow traces have been received yet.</p>}</div></Panel></div>
  </>;
}

function SessionsView({ analytics }: { analytics: Analytics }) {
  return <>
    <PageIntro title="Sessions" desc="Inspect each MCP connection as a runtime event: who connected, how long it lasted, and what happened." />
    <div className="grid gap-4 sm:grid-cols-3"><Metric label="Active connections" value={fmt(analytics.sessions)} icon={Activity} /><Metric label="Tool calls" value={fmt(analytics.tool_calls)} icon={Wrench} /><Metric label="Session completion" value={percent(analytics.completion_rate)} icon={Check} /></div>
    <div className="mt-6"><Panel title="Connection log" subtitle="One row per observed MCP session"><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="border-b border-line text-[10px] font-semibold uppercase tracking-[0.08em] text-faint"><tr><th className="pb-3">Client</th><th className="pb-3">Session</th><th className="pb-3">Started</th><th className="pb-3">Duration</th><th className="pb-3">Calls</th><th className="pb-3">Status</th></tr></thead><tbody className="divide-y divide-line">{analytics.workflows.length ? analytics.workflows.map((session) => <tr key={session.session_id}><td className="py-4"><div className="flex items-center gap-2.5"><ClientTile name={clientMark(session.client_name)} className="h-7 w-7 rounded-full" /><span className="text-ink">{session.client_name}</span></div></td><td className="py-4 font-mono text-xs text-muted">{session.session_id.slice(0, 18)}...</td><td className="py-4 text-xs text-muted">{new Date(session.started_at).toLocaleString()}</td><td className="py-4 text-muted">{session.duration_ms}ms</td><td className="py-4 text-muted">{session.calls}</td><td className="py-4"><span className={session.completed ? "text-brand-strong" : "text-amber-700"}>{session.completed ? "Completed" : "Stopped"}</span></td></tr>) : <tr><td colSpan={6} className="py-10 text-center text-sm text-muted">No sessions have been received yet.</td></tr>}</tbody></table></div></Panel></div>
  </>;
}
...
function ClientsView({ analytics }: { analytics: Analytics }) {
  ...
  return <>
    <PageIntro title="Clients" desc="Compare the AI products and internal agents that connect to your server, then see where adoption is concentrated." />
