...
export function DashboardApp({
  ...
  useEffect(() => {
    ...
    const requestedTrace = params.get("view") === "trace" ? params.get("session_id") : null;
    if (requestedTrace) {
      // URL state is read after hydration so the server-rendered dashboard stays stable.
      window.setTimeout(() => { setTraceSessionId(requestedTrace); setView("sessions"); }, 0);
    }
  }, []);
  const openTrace = (sessionId: string) => {
    setTraceSessionId(sessionId);
    setView("sessions");
    window.history.pushState({}, "", `/dashboard?view=trace&session_id=${encodeURIComponent(sessionId)}`);
  };
  const closeTrace = () => {
...
function ViewContent({ view, analytics, keys, working, onGenerateKey, onRevokeKey, onViewChange, onViewTrace, isSample }: { view: View; analytics: Analytics; keys: Key[]; working: boolean; onGenerateKey: () => void; onRevokeKey: (id: string) => void; onViewChange: (view: View) => void; onViewTrace: (sessionId: string) => void; isSample: boolean }) {
...
function WorkflowsView({ analytics, sessionsOnly, onViewTrace, isSample }: { analytics: Analytics; sessionsOnly: boolean; onViewTrace: (sessionId: string) => void; isSample: boolean }) {
  if (sessionsOnly) {
    return <SessionsView analytics={analytics} onViewTrace={onViewTrace} isSample={isSample} />;
  }
  ...
  return <>
    ...
    <div className="mt-6"><Panel title="Observed workflow traces" subtitle="Recent tool sequences attached to a connection"><div className="mt-3 divide-y divide-line">{analytics.workflows.length ? analytics.workflows.slice(0, 8).map((workflow) => <div key={workflow.session_id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><p className="font-mono text-xs text-ink">{workflow.tools.filter(Boolean).join(" → ") || "No tool sequence"}</p><p className="mt-1 text-[11px] text-muted">{workflow.client_name} · {workflow.calls} calls · {workflow.duration_ms}ms · {workflow.completion_source === "workflow_events" ? "explicit workflow signal" : "session heuristic"}</p></div><div className="flex items-center gap-3"><span className={`text-[11px] font-medium ${workflow.completed ? "text-brand-strong" : "text-amber-700"}`}>{workflow.completion_source === "workflow_events" ? (workflow.completed ? "Explicitly completed" : "Explicitly failed") : workflow.completed ? "Ended after successful call" : "Ended without explicit outcome"}</span>{!isSample && <button onClick={() => onViewTrace(workflow.session_id)} className="text-xs font-medium text-brand-strong hover:underline">View trace</button>}</div></div>) : <p className="py-8 text-sm text-muted">No workflow traces have been received yet.</p>}</div></Panel></div>
  </>;
}

function SessionsView({ analytics, onViewTrace, isSample }: { analytics: Analytics; onViewTrace: (sessionId: string) => void; isSample: boolean }) {
  return <>
    <PageIntro title="Sessions" desc="Inspect each observed MCP connection: who connected, how long it lasted, and which signals were recorded." />
    ...
    <div className="mt-6"><Panel title="Connection log" subtitle="One row per observed MCP session"><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="border-b border-line text-[10px] font-semibold uppercase tracking-[0.08em] text-faint"><tr><th className="pb-3">Client</th><th className="pb-3">Session</th><th className="pb-3">Started</th><th className="pb-3">Duration</th><th className="pb-3">Calls</th><th className="pb-3">Correlation</th><th className="pb-3">Status</th><th className="pb-3">Trace</th></tr></thead><tbody className="divide-y divide-line">{analytics.workflows.length ? analytics.workflows.map((session) => <tr key={session.session_id}><td className="py-4"><div className="flex items-center gap-2.5"><ClientTile name={clientMark(session.client_name)} className="h-7 w-7 rounded-full" /><span className="text-ink">{session.client_name}</span></div></td><td className="py-4 font-mono text-xs text-muted">{session.session_id.slice(0, 18)}...</td><td className="py-4 text-xs text-muted">{new Date(session.started_at).toLocaleString()}</td><td className="py-4 text-muted">{session.duration_ms}ms</td><td className="py-4 text-muted">{session.calls}</td><td className="py-4 text-xs text-muted">{session.correlation_quality === "session_id" ? "Session ID" : session.correlation_quality === "transport_generated" ? "Transport-generated" : session.correlation_quality === "missing" ? "Missing/degraded" : "Mixed"}</td><td className="py-4"><span className={session.completed ? "text-brand-strong" : "text-amber-700"}>{session.completion_source === "workflow_events" ? (session.completed ? "Explicitly completed" : "Explicitly failed") : session.completed ? "Ended after successful call" : "Ended without explicit outcome"}</span><span className="mt-1 block text-[10px] text-faint">{session.completion_source === "workflow_events" ? "explicit workflow" : session.completion_source === "session_heuristic" ? "session heuristic" : "no signal"}</span></td><td className="py-4">{isSample ? <span className="text-[11px] text-faint">Sample only</span> : <button onClick={() => onViewTrace(session.session_id)} className="text-xs font-medium text-brand-strong hover:underline">View trace</button>}</td></tr>) : <tr><td colSpan={8} className="py-10 text-center text-sm text-muted">No sessions have been received yet.</td></tr>}</tbody></table></div></Panel></div>
  </>;
}
