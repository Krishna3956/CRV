...
export default function DashboardPage() {
  ...
  const createWorkspace = async () => {
    ...
    try { const response = await fetch("/api/v1/account/workspace", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" }); const body = await response.json(); if (!response.ok) throw new Error(body.error || "Could not create your workspace."); setNewKey(body.api_key); const account = await loadAccount(); if (account.workspace) await loadAnalytics(); }
