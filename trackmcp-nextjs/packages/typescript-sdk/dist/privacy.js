...
function marker(reason, originalType, extra = {}) {
...
function resourceMarker(value) {
    ...
    if (/(?:authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|password|secret)\s*[:=]\s*\S+/i.test(value))
        return marker("credential", "string");
    if (/\bbearer\s+[A-Za-z0-9._~+/-]+=*/i.test(value))
        return marker("resource_uri", "string");
    ...
    try {
        ...
        if (hasCredentials || hasSensitiveQuery)
            return marker("resource_uri", "string");
    }
    catch {
        // Not a URL; it can still be an ordinary string or base64 value.
    }
    if (likelyBase64(value))
        return marker("binary", "string", { original_bytes: Math.floor(value.length * 0.75) });
    return undefined;
}
...
function sanitizeValue(value, depth, path, options, explicitPaths, extraKeys, seen) {
    ...
    if (typeof value !== "object")
        return marker("binary", typeof value);
    if (seen.has(value))
        return marker("circular", originalType(value));
    if (depth >= options.maxPayloadDepth)
        return marker("max_payload_depth", originalType(value));
    seen.add(value);
    try {
        if (Array.isArray(value)) {
            ...
            if (value.length > options.maxPayloadKeys)
                result.push(marker("max_payload_keys", "array"));
            return result;
        }
        const record = value;
        ...
        for (const key of keys.slice(0, options.maxPayloadKeys)) {
            ...
            try {
                if (isSensitiveKey(key, extraKeys))
                    ...
                    result[key] = sanitizeValue(record[key], depth + 1, childPath, options, explicitPaths, extraKeys, seen);
            }
            catch {
                result[key] = marker("binary", "unknown");
            }
        }
        if (keys.length > options.maxPayloadKeys)
            result[TRUNCATION_MARKER_KEY] = marker("max_payload_keys", "object");
        return result;
    }
    finally {
        seen.delete(value);
...
export function sanitizePayload(value, options) {
    if (options.mode === "metadata" || value === undefined)
        return undefined;
    const limits = {
