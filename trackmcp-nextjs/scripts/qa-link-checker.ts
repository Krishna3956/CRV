...
async function checkLink(link: string, baseUrl: string): Promise<LinkCheckResult> {
  try {
...
// Check a single page
async function checkPage(page: string, baseUrl: string): Promise<PageCheckResult> {
  try {
    ...
    console.log(`   ❌ Error checking page: ${error instanceof Error ? error.message : 'Unknown error'}`)
    return {
      page,
      links: [],
      brokenLinks: [],
...
// Check all internal links in database
async function checkDatabaseLinks(): Promise<void> {
  console.log('\n🔗 Checking database links...\n')

  // Check tool pages
  console.log('📋 Checking tool pages...')
  ...
  let validToolLinks = 0
  let invalidToolLinks = 0

  for (const tool of tools || []) {
    if (tool.github_url) {
      try {
        const response = await fetch(tool.github_url, { method: 'HEAD' })
        if (response.ok) {
          validToolLinks++
        } else {
          console.log(`   ❌ Invalid GitHub URL: ${tool.github_url} (${response.status})`)
          invalidToolLinks++
        }
      } catch (error) {
        console.log(`   ❌ Error checking GitHub URL: ${tool.github_url}`)
        invalidToolLinks++
      }
    }
  }

  console.log(`   ✅ Valid tool links: ${validToolLinks}`)
  console.log(`   ❌ Invalid tool links: ${invalidToolLinks}`)
}
